  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl p-5 border border-slate-200 shadow-md space-y-5">
          
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <span className="text-xs font-extrabold text-slate-900 flex items-center gap-1.5">
              <Sliders className="w-4 h-4 text-purple-600" /> Farm Field Inputs
            </span>
          </div>

          {/* Select Crop */}
          <div className="space-y-1.5">
            <label className="text-xs text-slate-900 block font-bold">Select Target Crop</label>
            <select
              value={cropName}
              onChange={(e) => setCropName(e.target.value)}
              className="w-full bg-slate-100 border border-slate-300 text-slate-900 text-xs font-bold rounded-xl px-3 py-2 focus:outline-none focus:border-agri-500 shadow-sm cursor-pointer"
            >
              {cropOptions.map((c) => (
          ...
          {/* District Selector */}
          <div className="space-y-1.5">
            <label className="text-xs text-slate-900 block font-bold">Target Karnataka District</label>
            <select
              value={districtId}
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="text-slate-900 font-bold">Land Area (Acres)</span>
              <span className="font-mono font-extrabold text-agri-700">{acres} Acres</span>
            </div>
            <input
              type="range"
              min="1"
              ...
              className="w-full accent-agri-600 bg-slate-200 h-2.5 rounded-lg cursor-pointer"
            />
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="text-slate-900 font-bold">Soil Health Index</span>
              <span className="font-mono font-extrabold text-amber-600">{soilHealth}%</span>
            </div>
            <input
              type="range"
              min="40"
              ...
              className="w-full accent-amber-600 bg-slate-200 h-2.5 rounded-lg cursor-pointer"
            />
          </div>

          {/* Irrigation Method */}
          <div className="space-y-1.5">
            <label className="text-xs text-slate-900 block font-bold">Irrigation Method</label>
          ...
          {/* Fertilizer Practice */}
          <div className="space-y-1.5">
            <label className="text-xs text-slate-900 block font-bold">Fertilizer Practice</label>
            <div className="grid grid-cols-3 gap-2 text-[11px]">
              {(["Organic", "Standard Balanced", "Excessive Chemical"] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setFertilizer(f)}
                  className={`py-2 px-1 rounded-xl font-bold border transition-all truncate ${
                    fertilizer === f
          ...
          {forecast && (
            <div className="bg-white rounded-3xl p-6 border border-purple-200 shadow-xl space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
                <div>
                  <span className="text-xs text-purple-700 font-bold uppercase tracking-wider block">Forecast Output</span>
                  <h3 className="text-xl md:text-2xl font-extrabold text-slate-900 mt-0.5">
                    {cropName} Yield Projection
                  </h3>
                  <span className="text-xs text-slate-600 font-medium">Total Farm Area: {acres} Acres</span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-5 rounded-2xl bg-slate-100 border border-slate-200 space-y-2">
                  <span className="text-xs text-slate-600 block font-semibold">Estimated Total Harvest Yield</span>
                  ...
                  <span className="text-xs text-slate-600 block pt-1 font-medium">
                    ({forecast.expectedYieldPerAcre} Quintals / Acre)
                  </span>
                </div>

                <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-200 space-y-2">
                  <span className="text-xs text-emerald-800 block font-semibold">Estimated Revenue Potential</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-mono font-extrabold text-emerald-700">
                      ₹{forecast.estimatedRevenueINR.toLocaleString()}
                    </span>
                  </div>
                  <span className="text-xs text-emerald-700 block pt-1 font-medium">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3 text-xs">
                <div className="flex justify-between items-center text-slate-800 font-medium">
                  ...
                  <span className="font-mono font-bold text-slate-900">{forecast.benchmarkAverageQuintals} Quintals / Acre</span>
                </div>
                <div className="flex justify-between items-center text-slate-800 font-medium">
                  <span>Predicted Precision AI Yield:</span>
                  <span className="font-mono font-bold text-agri-700">{forecast.expectedYieldPerAcre} Quintals / Acre</span>
                </div>
              </div>

              {/* Farming Action Suggestions */}
              <div className="space-y-2">
                <span className="text-xs font-bold text-slate-900 block">Smart Precision Recommendations:</span>
                <div className="space-y-2">
